...
const VarianceCell = ({ value }) => {
  if (value === null || value === undefined) return <td className="border p-2 text-center text-gray-400">—</td>;
  const num = parseFloat(value);
  const color = num < 0 ? 'text-red-500' : num > 0 ? 'text-green-600' : 'text-gray-500';
  return <td className={`border p-2 text-center font-medium ${color}`}>{value}%</td>;
};
...
const SizeScheduleEffortPage = () => {
  ...
  const phasesWithData = useMemo(() => {
    return phases.map(phase => {
      const cumulative = cumulativeEfforts.find(c => c.id === phase.id) || {};
      return {
        ...phase,
        ...
        cumulativeEstimatedEffort: cumulative.cumulativeEstimatedEffort || 0,
        cumulativeRevisedEffort:   cumulative.cumulativeRevisedEffort   || 0,
        cumulativeActualEffort:    cumulative.cumulativeActualEffort    || 0,
      };
    });
  ...
  const renderCell = (phase, col) => {
    switch (col.type) {
      ...
      case 'variance':
        return <VarianceCell key={col.key} value={phase[col.key]} />;
      case 'readonly':
      default:
        return (
          <td key={col.key} className="border p-2 text-center text-gray-600">
            {phase[col.key] ?? '—'}
          </td>
        );
    }
  ...
  return (
    <div>
      <PageHeader
        title="Size, Schedule & Effort"
        subtitle="Phase-wise tracking with schedule & effort variances"
      />
        ...
        <div className="flex items-center justify-between px-4 py-2 bg-gray-50 border-b border-gray-200">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium rounded border border-gray-300 bg-white text-gray-700
                         hover:bg-blue-500 hover:text-white hover:border-blue-500 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
              ← Prev
            </button>
            <button
          ...
          <table className="w-full border-collapse text-sm">
            <thead className="bg-gray-100">
              ...
              <tr className="border-b">
                <th
                  rowSpan={2}
                  className="min-w-[150px] border p-2 align-middle text-left sticky left-0 bg-gray-100 z-10"
                >
                  Project Phases
                </th>
                {section.groups.map((g, i) => (
                  <th
                    key={i}
                    colSpan={g.cols}
                    className="border p-2 text-center bg-blue-50 text-blue-700 font-semibold"
                  >
                    {g.label}
                  </th>
                ))}
              ...
              <tr>
                {section.cols.map(col => (
                  <th
                    key={col.key}
                    className="border p-2 text-center font-medium text-gray-600 whitespace-nowrap"
                  >
                    {col.label}
                  </th>
                ))}
            ...
            <tbody>
              {phasesWithData.map(phase => (
                <tr key={phase.id} className="hover:bg-gray-50">
                  {/* Sticky phase name */}
                  <td className="border p-2 font-semibold sticky left-0 bg-white z-10 text-gray-800">
                    {phase.phase}
                  </td>
                  {/* Dynamic columns */}
                  {section.cols.map(col => renderCell(phase, col))}
                </tr>
              ))}
            </tbody>
          </table>
